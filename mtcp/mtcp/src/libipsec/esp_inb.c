	...
	struct rte_esp_tail *espt, uint32_t *hlen, uint32_t *tofs)
{
	...
	ofs = mb->pkt_len - tlen;
	// rider added 2020-12-9 add 8 bytes for esp over udp
	hlen[0] = mb->l2_len + mb->l3_len + 8;
...
// rider added 2020-9-30
uint16_t
inline_inb_tun_pkt_process_custom(struct rte_ipsec_sa *sa,
	struct rte_mbuf *mb[], uint16_t num)
{
	return esp_inb_pkt_process(sa, mb, num, 0, tun_process);
}
//
