...
void handlecasesession(int conn_id, void* request, char* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecasesession function started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr;
    pkt.clear_pkt();
    int packet_length;
    ...
    // printf("RIDER REPORT handlecasesession function finished\n"); fflush(stdout);
}
void handlecasesession_get(int conn_id, void* request, void* packet, int unused1, int unused2){

    // printf("RIDER REPORT handlecasesession_get function started\n"); fflush(stdout);
	Packet pkt;
...
//	string saddr;
    ...
    // cout << "RIDER REPORT: handlecasesession_get function 22222 " << sgw_ip << ", " << sgw_s11_portnum << endl; fflush(stdout);
    ...
    // cout << "RIDER REPORT: handlecasesession_get function 33333 " << ", k_nas_int=" << k_nas_int << endl; fflush(stdout);
	if (HMAC_ON) {
		res = g_integrity.hmac_check(pkt, k_nas_int);
		if (res == false) {
				cout << "mme_handlesecuritymodecomplete:" << " hmac failure1: " << x->guti << endl;
	...
	pkt2.prepend_gtp_hdr(2, 1, pkt2.len, 0);
	pkt2.prepend_len();
...
//        registerCallback(x->initial_fd, READ, handlecaseauth);
    ...
    // printf("RIDER REPORT handlecasesession_get function finished\n"); fflush(stdout);
}

void handlecasesgwreply1(int conn_id, void* request, char* packet0, int unused1, int unused2){
    char packet[4096];
    memcpy(packet, packet0, unused1);

    // printf("RIDER REPORT handlecasesgwreply1 function started\n"); fflush(stdout);
    Packet pkt;
        char * dataptr; // Pointer to data for copying to packet
        uint64_t imsi;
        int returnval; // Simple address
    ...
    string s_add;
        mdata *x = static_cast<mdata*>(request);
    // printf("RIDER REPORT handlecasesgwreply1 function 11111\n"); fflush(stdout);
    unsetPktDNE(conn_id, (void*)x->session_pkt);
    // printf("RIDER REPORT handlecasesgwreply1 function 11111-1 %x,%x,%x,%x\n", packet[0], packet[1], packet[2], packet[3]); fflush(stdout);
        pkt.clear_pkt();
        memmove(&packet_length, packet, sizeof(int)); // Move packet length into packet_len
        // printf("RIDER REPORT handlecasesgwreply1 function 11111-2, conn_id=%d, packet_length=%d ,%x,%x,%x,%x\n",conn_id , packet_length, packet[packet_length+4], packet[packet_length+5], packet[packet_length+6], packet[packet_length+7]); fflush(stdout);
        if(packet_length <= 0)
        {
            perror("Error in reading packet_length\n");
            cout<<errno<<endl;
        }
        // printf("RIDER REPORT handlecasesgwreply1 function 11111-3\n"); fflush(stdout);
        pkt.clear_pkt();
        dataptr = packet+sizeof(int);
        // printf("RIDER REPORT handlecasesgwreply1 function 11111-4\n"); fflush(stdout);
        memcpy(pkt.data, (dataptr), packet_length);
        // printf("RIDER REPORT handlecasesgwreply1 function 11111-5\n"); fflush(stdout);
        pkt.data_ptr = 0;
        pkt.len = packet_length;
        //pkt.extract_s1ap_hdr();
    // printf("RIDER REPORT handlecasesgwreply1 function 22222\n"); fflush(stdout);
    pkt.extract_gtp_hdr();
    ...
    TRACE(cout<<"data received from SGW"<<x->guti<<endl;)
    // printf("RIDER REPORT handlecasesgwreply1 function 33333\n"); fflush(stdout);
    ...
    // printf("RIDER REPORT handlecasesgwreply1 function finished\n"); fflush(stdout);
}
void handlecasesgwreply1_get(int conn_id, void* request, void* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecasesgwreply1_get function started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr; // Pointer to data for copying to packet
    uint64_t imsi;
    int returnval; // Simple address
    ...
    UeContext current_context; // Stores current UEContext
    //cout<< "in cse3_get "<<packet<<endl;
    // printf("RIDER REPORT handlecasesgwreply1_get function 11111\n"); fflush(stdout);
    memcpy(&current_context, packet, sizeof(UeContext));
    // printf("RIDER REPORT handlecasesgwreply1_get function 22222\n"); fflush(stdout);
    ...
    pkt.prepend_len();
    // printf("RIDER REPORT handlecasesgwreply1_get function 33333\n"); fflush(stdout);
    char* pkt1 = getPktBuf(conn_id);
    // printf("RIDER REPORT handlecasesgwreply1_get function 44444\n"); fflush(stdout);
    memcpy((void*)pkt1, (void*)(pkt.data), pkt.len);
    // printf("RIDER REPORT handlecasesgwreply1_get function 55555\n"); fflush(stdout);
    ...
    // printf("RIDER REPORT handlecasesgwreply1_get function finished\n"); fflush(stdout);
}

void handlecaseeps(int conn_id, void* request, char* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecaseeps function started\n"); fflush(stdout);
	Packet pkt;
        char * dataptr;
        pkt.clear_pkt();
        int packet_length;
    ...
    // printf("RIDER REPORT handlecaseeps function finished\n"); fflush(stdout);
}
void handlecaseeps_get(int conn_id, void* request, void* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecaseeps_get function started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr; // Pointer to data for copying to packet
    uint64_t imsi;
    int returnval; // Simple address
    ...
    pkt.clear_pkt();
    memmove(&packet_length, (x->session_pkt), sizeof(int)); // Move packet length into packet_len
    // printf("RIDER REPORT handlecaseeps_get function 11111 packet_length=%d\n", packet_length); fflush(stdout);
    if(packet_length <= 0)
    {
        perror("Error in reading packet_length\n");
        cout<<errno<<endl;
    ...
    // printf("RIDER REPORT handlecaseeps_get function 22222\n"); fflush(stdout);
    pkt.extract_item(x->eps_bearer_id);
    pkt.extract_item(x->s1_uteid_dl);
    // printf("RIDER REPORT handlecaseeps_get function 33333\n"); fflush(stdout);
    //g_sync.mlock(uectx_mux);
    current_context.s1_uteid_dl = x->s1_uteid_dl;
    current_context.emm_state = 1;
    setData(conn_id, "ue_state", x->guti, LOCAL,  &current_context, sizeof(UeContext), NULL);
    // printf("RIDER REPORT handlecaseeps_get function 44444\n"); fflush(stdout);
    //g_sync.munlock(uectx_mux);
    ...
    // printf("RIDER REPORT handlecaseeps_get function 44444-1\n"); fflush(stdout);
    Packet pkt2;
    pkt2.clear_pkt();
    ...
    pkt2.prepend_len();
    // printf("RIDER REPORT handlecaseeps_get function 44444-2 pkt2.len=%d conn_id=%d\n", pkt2.len, conn_id); fflush(stdout);
    char* pkt1 = getPktBuf(conn_id);
    // printf("RIDER REPORT handlecaseeps_get function 44444-3 pkt2.len=%d\n", pkt2.len); fflush(stdout);
    memcpy((void*)pkt1, (void*)(pkt2.data), pkt2.len);
    // printf("RIDER REPORT handlecaseeps_get function 55555\n"); fflush(stdout);
    TRACE(cout<<"EPS data sent to SGW"<<x->guti<<endl;)
    registerCallback(x->sgw_fd, READ, handlecaseeps_sgwreply);
    // unsetPktDNE(conn_id, (void*)x->session_pkt);
//        registerCallback(x->initial_fd, READ, handlecaseauth);
    sendData(x->sgw_fd, pkt1, pkt2.len);
    // printf("RIDER REPORT handlecaseeps_get function finished\n"); fflush(stdout);
}
void handlecaseeps_sgwreply(int conn_id, void* request, char* packet0, int unused1, int unused2){
    char packet[4096];
    memcpy(packet, packet0, unused1);

    // printf("RIDER REPORT handlecaseeps_sgwreply function started, unused1=%d, unused2=%d\n", unused1, unused2); fflush(stdout);
    Packet pkt;
        char * dataptr; // Pointer to data for copying to packet
        uint64_t imsi;
        int returnval; // Simple address
    ...
    unsetPktDNE(conn_id, (void*)x->session_pkt);
        pkt.clear_pkt();
        // printf("RIDER REPORT handlecaseeps_sgwreply function 11111 %x,%x,%x,%x\n", packet[0], packet[1], packet[2], packet[3]); fflush(stdout);
        memmove(&packet_length, packet, sizeof(int)); // Move packet length into packet_len
        // printf("RIDER REPORT handlecaseeps_sgwreply function 11111-1, conn_id=%d, packet_length=%d, %x,%x,%x,%x\n",conn_id , packet_length, packet[0], packet[1], packet[2], packet[3]); fflush(stdout);
        if(packet_length <= 0)
        {
            perror("Error in reading packet_length\n");
            cout<<errno<<endl;
        }
        // printf("RIDER REPORT handlecaseeps_sgwreply function 22222\n"); fflush(stdout);
        pkt.clear_pkt();
        dataptr = packet+sizeof(int);
        memcpy(pkt.data, (dataptr), packet_length);
        pkt.data_ptr = 0;
    ...
    // printf("RIDER REPORT handlecaseeps_sgwreply function finished\n"); fflush(stdout);

}
void handlecaseeps_sgwreply_get(int conn_id, void* request, void* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecaseeps_sgwreply_get function started conn_id=%d\n", conn_id); fflush(stdout);
    Packet pkt;
    char * dataptr; // Pointer to data for copying to packet
    uint64_t imsi;
    int returnval; // Simple address
    ...
    UeContext current_context; // Stores current UEContext
    //cout<< "in cse3_get "<<packet<<endl;
    // printf("RIDER REPORT handlecaseeps_sgwreply_get 111111 unused1=%d, sizeof(UeContext)=%d\n", unused1, sizeof(UeContext)); fflush(stdout);
    memcpy(&current_context, packet, sizeof(UeContext));
    // printf("RIDER REPORT handlecaseeps_sgwreply_get 222222\n"); fflush(stdout);
    ...
    //registerCallback(x->sgw_fd, READ, handlecasesgwreply1);
    sendData(x->initial_fd, pkt1, pkt.len);
    // printf("RIDER REPORT handlecaseeps_sgwreply_get function finished\n"); fflush(stdout);
}

void handlecasedetach(int conn_id, void* request, char* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecasedetach function started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr;
    pkt.clear_pkt();
    int packet_length;
    ...
    // printf("RIDER REPORT handlecasedetach function finished\n"); fflush(stdout);
}
void handlecasedetach_get(int conn_id, void* request, void* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecasedetach_get function started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr; // Pointer to data for copying to packet
    uint64_t imsi;
    int returnval; // Simple address
    ...
    pkt.clear_pkt();
    memmove(&packet_length, (x->session_pkt), sizeof(int)); // Move packet length into packet_len
    // printf("RIDER REPORT handlecasedetach_get 111111 packet_length=%d\n", packet_length); fflush(stdout);
    if(packet_length <= 0)
    {
        perror("Error in reading packet_length\n");
        cout<<errno<<endl;
...
//        registerCallback(x->initial_fd, READ, handlecaseauth);
    ...
    // printf("RIDER REPORT handlecasedetach_get function finished\n"); fflush(stdout);
}

void handlecasedetach_sgwreply(int conn_id, void* request, char* packet0, int unused1, int unused2){
    char packet[4096];
    memcpy(packet, packet0, unused1);
    
    // printf("RIDER REPORT handlecasedetach_sgwreply function started\n"); fflush(stdout);
	Packet pkt;
        char * dataptr; // Pointer to data for copying to packet
        uint64_t imsi;
        int returnval; // Simple address
    ...
    // printf("RIDER REPORT handlecasedetach_sgwreply function finished\n"); fflush(stdout);

}
void handlecasedetach_sgwreply_get(int conn_id, void* request, void* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecasedetach_sgwreply_get function started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr; // Pointer to data for copying to packet
    uint64_t imsi;
    int returnval; // Simple address
    ...
    guti = x->guti;
    freeReqObj(conn_id, 1);
    // rider added 2021-1-7 disabled closeconn as it made some duplicated send of packet
    // closeConn(conn_id);
    UeContext current_context; // Stores current UEContext
    //cout<< "in cse3_get "<<packet<<endl;
    memcpy(&current_context, packet, sizeof(UeContext));
    ...
    // rider added 2021-1-7 restart from begining
    // freeReqObj(ran_fd, 1);
    // closeConn(ran_fd);
    registerCallback(x->initial_fd, READ, handleRegistrationRequest);
    // printf("RIDER REPORT handlecasedetach_sgwreply_get function finished\n"); fflush(stdout);
}

void handlecaseauth(int conn_id, void* request, char* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecaseauth started\n"); fflush(stdout);
	Packet pkt;
        char * dataptr; // Pointer to data for copying to packet
        uint64_t imsi;
        int returnval; // Simple address
    ...
    // printf("RIDER REPORT handlecaseauth finished\n"); fflush(stdout);
}
void handlecaseauth_get(int conn_id, void* request, void* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecaseauth_get started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr; // Pointer to data for copying to packet
    uint64_t imsi;
    int returnval; // Simple address
...
//        registerCallback(x->initial_fd, READ, handlecaseauth);
    sendData(x->initial_fd, pkt1, pkt.len);
    // printf("RIDER REPORT handlecaseauth_get finished\n"); fflush(stdout);
}

void handlecase3(int conn_id, void* request, char* packet, int unused1, int unused2){

    // printf("RIDER REPORT handlecase3 function started\n"); fflush(stdout);
	Packet pkt;
	char * dataptr; // Pointer to data for copying to packet
	uint64_t imsi;
	int returnval; // Simple address
    ...
    // printf("RIDER REPORT handlecase3 function finished\n"); fflush(stdout);
}
void handlecase3_get(int conn_id, void* request, void* packet, int unused1, int unused2){
    // printf("RIDER REPORT handlecase3_get started\n"); fflush(stdout);
    Packet pkt;
    char * dataptr; // Pointer to data for copying to packet
    uint64_t imsi;
    int returnval; // Simple address
    ...
    UeContext current_context; // Stores current UEContext
    //cout<< "in cse3_get "<<packet<<endl;
    // printf("RIDER REPORT handlecase3_get 11111 unused1=%d, sizeof(UeContext)=%d\n", unused1, sizeof(UeContext)); fflush(stdout);
    memcpy(&current_context, packet, sizeof(UeContext));
    // printf("RIDER REPORT handlecase3_get 22222\n"); fflush(stdout);
    ...
    mme_s1ap_ue_id = x->msui;
    // printf("RIDER REPORT handlecase3_get 33333 x->autn_num=%x, x->rand_num=%x\n", x->autn_num, x->rand_num); fflush(stdout);
    pkt.clear_pkt();
    ...
    registerCallback(x->initial_fd, READ, handlecaseauth);
    sendData(x->initial_fd, pkt1, pkt.len);
    // printf("RIDER REPORT handlecase3_get finished\n"); fflush(stdout);
}

void handleRegistrationRequest(int conn_id, void* request, char* packet, int unused1, int unused2){

    // printf("\n\n\n\nRIDER REPORT: handleRegistrationRequest function started, unused1=%d\n", unused1); fflush(stdout);
    // printf("RIDER REPORT handleRegistrationRequest function 11111 %x,%x,%x,%x\n", packet[0], packet[1], packet[2], packet[3]); fflush(stdout);
    ...
    if(packet_length <= 0)
        ...
        if (pkt.s1ap_hdr.mme_s1ap_ue_id == 0) {
            // printf("RIDER REPORT handleRegistrationRequest function 11111\n"); fflush(stdout);
        	num_autn_vectors = 1;
            ...
            //cout << "before sendData" << endl;
            //cout << "before sendData" << endl;
            // printf("RIDER REPORT handleRegistrationRequest function 22222, hss_fd=%d, pkt.len=%d, %x,%x,%x,%x\n", hss_fd, pkt.len, pkt1[0], pkt1[1], pkt1[2], pkt1[3]); fflush(stdout);
		    sendData(hss_fd, pkt1, pkt.len);
            //cout << "after all ......" << endl;
        }
    // printf("RIDER REPORT handleRegistrationRequest function finished\n"); fflush(stdout);
}
...
int main(int argc, char *argv[]) {
    ...
    initLibvnf(atoi(argv[1]), 1024, "169.254.9.10", dataStorePorts, 131072, false);
