...
void serverThreadFunc(void *arg1) {
    ...
    // printf("RIDER REPORT serverThreadFunc function 1111 sched_getcpu()=%d\n", sched_getcpu()); fflush(stdout);
    //step 2. mtcp_core_affinitize
    mtcp_core_affinitize(core);
    // printf("RIDER REPORT serverThreadFunc function 2222 sched_getcpu()=%d\n", sched_getcpu()); fflush(stdout);
    ...
    // printf("RIDER REPORT serverThreadFunc function 11111 p_core[id].mp_pkt_v.size()=%x, (p_core[id].mp_pkt_v).size()=%x\n", p_core[id].mp_pkt_v.size(), (p_core[id].mp_pkt_v).size()); fflush(stdout);
    ...
    while (!done[id]) {
        ...
        for (int i = 0; i < nevents; i++) {
            if ((events[i].events & MTCP_EPOLLERR) ||
                ...
                // cout << "RIDER REPORT: mtcp_epoll_wait retrun : " << i << ", " << (events[i].events & MTCP_EPOLLERR) << ", " << (events[i].events & MTCP_EPOLLRDHUP) << ", " << (events[i].events & MTCP_EPOLLHUP) << ", " << strerror(err) << ", " << err << endl;
...
//		if (useRemoteDataStore_l) {
                if (events[i].data.sockid == p_core[id].ds_sockid || events[i].data.sockid == p_core[id].ds_sockid1) {
                    ...
                    mp_lock.unlock();
                    // printf("RIDER REPORT pointer track: 3333 %d, %x malloc\n", id, data); fflush(stdout);
                    int rd = mtcp_read(mctx, events[i].data.sockid, data, bufferSize);
                    if (rd <= 0) {
                        mtcp_epoll_ctl(mctx, ep, MTCP_EPOLL_CTL_DEL, events[i].data.sockid, NULL);
                        int close_ret = mtcp_close(mctx, events[i].data.sockid);
                        // printf("RIDER REPORT pointer track: 4444 %d, %x free\n", id, data); fflush(stdout);
                    ...
                    if (got1 == p_core[id].pkt_ptr.end()) {
                        // printf("RIDER REPORT pointer track: 5555 %d, %x free\n", id, data); fflush(stdout);
                ...
                while (!(p_core[id].queue_vec[events[i].data.sockid]).empty()) {
                    ...
                    // printf("RIDER REPORT pointer track: 6666 %d, %x free\n", id, data_to_send.data); fflush(stdout);
...
void unsetPktDNE(int vnf_connid, void *pkt_mem_ptr) {
    int id;
    id = vnf_connid / 10000000;
    // printf("RIDER REPORT pointer track: 7777 %d, %x free\n", id, pkt_mem_ptr); fflush(stdout);
...
char *getPktBuf(int vnf_connid) {
    int id;
    id = vnf_connid / 10000000;
    // printf("RIDER REPORT getPktBuf function 1111 id=%d\n", id); fflush(stdout);
    mp_lock.lock();
    char *pkt = static_cast<char *>(p_core[id].storagepkt.malloc());
    mp_lock.unlock();
    // printf("RIDER REPORT pointer track: 1111-1 %d, %x malloc\n", id, pkt); fflush(stdout);
    return pkt;
}
...
void sendData(int vnf_connid, char *packetToSend, int packet_size) {
    ...
    if (!p_core[id].queue_vec[connID].empty()) {
        ...
        // RIDER REPORT
        // printf("RIDER REPORT sendData function 11111 packet_size=%d\n", packet_size);
        // for (int i = 0; i < packet_size; i++) printf("%x,", packetToSend[i]); printf("\n"); fflush(stdout);

        ret = mtcp_write(mctx, connID, packetToSend, packet_size);
        // printf("RIDER REPORT sendData function 22222 ret=%d\n", ret); fflush(stdout);
        if (ret < 0) {

            // printf("RIDER REPORT sendData function 33333 ret=%d\n", ret); fflush(stdout);
            if (errno == ENOTCONN) {
                // printf("RIDER REPORT sendData function 44444 ret=%d\n", ret); fflush(stdout);
                queue_data data_to_send;
                data_to_send.data = packetToSend;
                data_to_send.data_len = packet_size;
                p_core[id].queue_vec[connID].push(data_to_send);
	...
	//	cout<<"done sending"<<endl;
                // printf("RIDER REPORT pointer track: 2222 %d, %x free\n", id, packetToSend); fflush(stdout);
