...
// rider added 2020-9-17 functions for dpdk control

int ConfigCopyFromEnclave(void *global_config_ptr);
int ConfigCopyToEnclave(void *global_config_ptr);
int SetNetEnv(char *dev_name_list, char *port_stat_list);
void dpdk_load_module(void);
void dpdk_init_handle(void *ctxt);
void dpdk_destroy_handle(void *ctxt);

static long sgx_ocall_dpdk_load_module(void *pms) {
    ms_ocall_dpdk_load_module_t* ms = (ms_ocall_dpdk_load_module_t*)pms;
    ODEBUG(OCALL_DPDK_LOAD_MODULE, ms);
    dpdk_load_module();
    return 0;
}

static long sgx_ocall_dpdk_init_handle(void *pms) {    
    ms_ocall_dpdk_init_handle_t* ms = (ms_ocall_dpdk_init_handle_t*)pms;
    ODEBUG(OCALL_DPDK_INIT_HANDLE, ms);
    dpdk_init_handle(ms->ms_ctxt_ptr);
    return 0;
}

static long sgx_ocall_dpdk_destroy_handle(void *pms) {
    ms_ocall_dpdk_destroy_handle_t* ms = (ms_ocall_dpdk_destroy_handle_t*)pms;
    ODEBUG(OCALL_DPDK_DESTROY_HANDLE, ms);
    dpdk_destroy_handle(ms->ms_ctxt_ptr);
    return 0;
}

static long sgx_ocall_set_net_env(void *pms) {
    ms_ocall_set_net_env_t* ms = (ms_ocall_set_net_env_t*)pms;
    ODEBUG(OCALL_SET_NET_ENV, ms);
    int ret;
    ConfigCopyFromEnclave(ms->ms_global_config_ptr);
    ret = SetNetEnv(ms->ms_dev_name_list, ms->ms_port_stat_list);
    ConfigCopyToEnclave(ms->ms_global_config_ptr);
    return ret;
}
//
...
sgx_ocall_fn_t ocall_table[OCALL_NR] = {
	...
	// rider added 2020-9-17 functions for dpdk control
    [OCALL_DPDK_LOAD_MODULE]    = sgx_ocall_dpdk_load_module,
    [OCALL_DPDK_INIT_HANDLE]    = sgx_ocall_dpdk_init_handle,
    [OCALL_DPDK_DESTROY_HANDLE] = sgx_ocall_dpdk_destroy_handle,
    [OCALL_SET_NET_ENV]         = sgx_ocall_set_net_env,
    //
};
