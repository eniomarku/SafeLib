...
void SctpClient::conn(string arg_server_ip_addr, int arg_server_port, struct sockaddr_in* my_addr) {
	init(arg_server_ip_addr, arg_server_port);
	// g_nw.set_rcv_timeout(conn_fd, 1);

	// rider added 2021-1-7 add bind to use static ports
	if (my_addr != NULL) {
		
		while (bind(conn_fd, (struct sockaddr*)my_addr, sizeof(struct sockaddr_in)) != 0) {
			printf("Unable to bind, retrying... port=%d, errno=%d\n", ntohs(my_addr->sin_port), errno);
			sleep(1);
		} 
	}
    //
...
void SctpClient::connect_with_server() {
	...
	TRACE(cout << "RIDER REPORT:" << " sctp_client: to_string(errno)=" << to_string(errno) << endl;)
