...
int Network::read_sctp_pkt(int conn_fd, Packet &pkt) {
	...
	if (retval > 0) {
		...
		printf("RIDER REPORT: Network::read_sctp_pkt function 11111 pkt_len=%d %x,%x,%x,%x\n", pkt_len, pkt.data[0], pkt.data[1], pkt.data[2], pkt.data[3]); fflush(stdout);
		pkt.clear_pkt();
...
int Network::write_sctp_pkt(int conn_fd, Packet pkt) {
	...
	printf("RIDER REPORT: Network::write_sctp_pkt function 11111 pkt.len=%d %x,%x,%x,%x\n", pkt.len, pkt.data[0], pkt.data[1], pkt.data[2], pkt.data[3]); fflush(stdout);
