...
string g_ran_ip_addr = "169.254.9.9"; // type 1
string g_trafmon_ip_addr = "169.254.9.9"; // type 1
string g_mme_ip_addr = "169.254.9.10"; // type 1
...
void Ran::conn_mme() {
	TRACE(cout << "RIDER REPORT:" << " ran: ran_ctx.enodeb_s1ap_ue_id=" << ran_ctx.enodeb_s1ap_ue_id << ", epc_addrs.mme_ip_addr=" << epc_addrs.mme_ip_addr << ", epc_addrs.mme_port=" << htons(epc_addrs.mme_port) << endl;)
	// rider added 2020-1-7 add my address and port to bind
	struct sockaddr_in my_addr;
    my_addr.sin_family = AF_INET; 
    my_addr.sin_addr.s_addr = INADDR_ANY; 
    my_addr.sin_port = htons(6006 + ran_ctx.enodeb_s1ap_ue_id); // use 6006, 6007, 6008, 6009
    my_addr.sin_addr.s_addr = inet_addr("169.254.9.9");
	//
	mme_client.conn(epc_addrs.mme_ip_addr, epc_addrs.mme_port, &my_addr);
	TRACE(cout << "RIDER REPORT:" << " ran: conn_mme finished" << endl;)
}
...
bool Ran::set_eps_session(TrafficMonitor &traf_mon) {
	...
	// rider added 2020-1-7 wait for confirmation for set session
	mme_client.rcv(pkt);
