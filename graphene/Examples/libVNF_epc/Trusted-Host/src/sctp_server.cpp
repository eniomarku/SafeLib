...
void SctpServer::worker_func(int worker_id) {
	...
	max_fd = pipe_fd;
	while (1) {
		printf("RIDER REPORT: SctpServer::worker_func 11111\n"); fflush(stdout);
		...
		if (FD_ISSET(pipe_fd, &fds)) {
			printf("RIDER REPORT: SctpServer::worker_func 22222\n"); fflush(stdout);
		...
		for (i = 0; i < conn_fds_size; ) {
			...
			if (FD_ISSET(conn_fds[i], &fds)) {
				printf("RIDER REPORT: SctpServer::worker_func 33333\n"); fflush(stdout);
				status = serve_client(conn_fds[i], worker_id);
				printf("RIDER REPORT: SctpServer::worker_func 44444\n"); fflush(stdout);
			}
			if (status == 0) {
				close(conn_fds[i]);
...
void SctpServer::accept_clients() {
	...
	while (1) {
		printf("RIDER REPORT: SctpServer::accept_clients function 11111 turn=%d\n", turn); fflush(stdout);
		conn_fd = accept(listen_fd, (struct sockaddr *)&client_sock_addr, &g_sock_addr_len);
		printf("RIDER REPORT: SctpServer::accept_clients function 22222 conn_fd=%d\n", conn_fd); fflush(stdout);
		g_utils.handle_type1_error(conn_fd, "Accept error: sctpserver_acceptclients");
		printf("RIDER REPORT: SctpServer::accept_clients function 33333\n"); fflush(stdout);
		pkt.clear_pkt();
		pkt.append_item(conn_fd);
		printf("RIDER REPORT: SctpServer::accept_clients function 44444 pipe_fds[turn][1]=%d\n", pipe_fds[turn][1]); fflush(stdout);
		status = g_nw.write_sctp_pkt(pipe_fds[turn][1], pkt);
		printf("RIDER REPORT: SctpServer::accept_clients function 55555 status=%d\n", status); fflush(stdout);
		g_utils.handle_type1_error(status, "Thread communication error: sctpserver_acceptclients");
		printf("RIDER REPORT: SctpServer::accept_clients function 66666\n"); fflush(stdout);
