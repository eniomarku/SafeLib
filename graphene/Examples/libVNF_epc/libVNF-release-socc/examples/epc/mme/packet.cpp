...
void Packet::prepend_s1ap_hdr(uint8_t msg_type, uint16_t msg_len, uint32_t enodeb_s1ap_ue_id, uint32_t mme_s1ap_ue_id) {
	...
	// printf("RIDER REPORT Packet::prepend_s1ap_hdr function started, msg_type=%d, msg_len=%d, enodeb_s1ap_ue_id=%d, mme_s1ap_ue_id=%d\n", msg_type, msg_len, enodeb_s1ap_ue_id, mme_s1ap_ue_id); fflush(stdout);
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);	
...
void Packet::prepend_diameter_hdr(uint8_t msg_type, uint16_t msg_len) {
	...
	// printf("RIDER REPORT Packet::prepend_diameter_hdr function started, msg_type=%d, msg_len=%d\n", msg_type, msg_len); fflush(stdout);
	uint8_t *tem_data = g_utils.allocate_uint8_mem(BUF_SIZE);	
