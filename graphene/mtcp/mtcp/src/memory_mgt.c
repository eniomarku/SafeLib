...
{
	printf("RIDER REPORT: MPCreate trusted started, chunk_size=%d, total_size=%ld\n", chunk_size, total_size); fflush(stdout);
	mem_pool_t mp;
	...
	if (geteuid() == 0) {
		if (mlock(mp->mp_startptr, total_size) < 0) {
			// rider added 2020-9-30 disable error log
...
{
	printf("RIDER REPORT: MPCreate untrusted started, name=%s, chunk_size=%d, total_size=%ld\n", name, chunk_size, total_size); fflush(stdout);
