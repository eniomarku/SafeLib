...
{
	int ret;
	ret = ocall_set_net_env(dev_name_list, port_stat_list, (void*)&CONFIG, sizeof(struct mtcp_config));
	// rider added 2021-1-7 get num_queues
	num_queues = MIN(CONFIG.num_cores, MAX_CPUS);
	printf("RIDER REPORT: SetNetEnv trusted CONFIG.num_cores=%d, num_queues=%d\n", CONFIG.num_cores, num_queues); fflush(stdout);
	//
	return ret;
}
/*----------------------------------------------------------------------------*/
...
FetchEndianType()
{
	// rider added 2021-1-7 force to return 0
	return 0;
	//
// rider added 2020-9-23 
//#ifndef DISABLE_DPDK
...
// rider added 2020-9-22 to copy CONFIG from/to enclave 
int
ConfigCopyFromEnclave(void *global_config_ptr) 
{
	if (global_config_ptr) {
		memcpy(&CONFIG, global_config_ptr, sizeof(struct mtcp_config));
		return 0;
	}
	return -1;
}
int 
ConfigCopyToEnclave(void *global_config_ptr) 
{
	if (global_config_ptr) {
		memcpy(global_config_ptr, &CONFIG, sizeof(struct mtcp_config));
		return 0;
	}
	return -1;
}
...
//#ifndef DISABLE_DPDK
		...
		for (int ii = 0; ii < 32; ii++) {printf("%c", cpumaskbuf[ii]);} printf("\n"); fflush(stdout);
		...
		num_queues = MIN(CONFIG.num_cores, MAX_CPUS);
		printf("RIDER REPORT: SetNetEnv untrusted CONFIG.num_cores=%d, num_queues=%d\n", CONFIG.num_cores, num_queues); fflush(stdout);
