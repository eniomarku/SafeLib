		...
		uint32_t cur_ts, uint8_t flags, uint8_t *payload, uint16_t payloadlen)
{
	...
	// printf("RIDER REPORT: tcp stream sending: source=%d:%d, dest=%d:%d, cpu=%d, g_stream_cnt=%d\n", cur_stream->saddr, cur_stream->sport, cur_stream->daddr, cur_stream->dport, mtcp->ctx->cpu, g_stream_cnt); fflush(stdout);
	int stream_exist = 0;
	for (int i = g_stream_cnt - 1; i >= 0; i--) {
		if (g_stream_infos[i].addr1 == cur_stream->saddr && g_stream_infos[i].port1 == cur_stream->sport && \
				g_stream_infos[i].addr2 == cur_stream->daddr && g_stream_infos[i].port2 == cur_stream->dport) {
			stream_exist = 1;
			break;
		}
	}
	if (stream_exist == 0) {
		g_stream_infos[g_stream_cnt].addr1 = cur_stream->saddr;
		g_stream_infos[g_stream_cnt].port1 = cur_stream->sport;
		g_stream_infos[g_stream_cnt].addr2 = cur_stream->daddr;
		g_stream_infos[g_stream_cnt].port2 = cur_stream->dport;
		g_stream_ctxt[g_stream_cnt] = (void*) mtcp->ctx;
		g_stream_cnt++;
	}
...
#ifdef IN_ENCLAVE
	tcph = (struct tcphdr *)IPOutputPlain(mtcp, cur_stream, 
			TCP_HEADER_LEN + optlen + payloadlen);
#else
	...
	// rider added 2020-9-29 ipsec encryption
#ifdef IN_ENCLAVE
	if (global_socket_ctx == NULL) { // inistalize socket context
		/* parse configuration file */
		if (parse_cfg_file(cfgfile) < 0) {
			rte_exit(EXIT_FAILURE, "Invalid cfg file\n");
		}
		global_socket_ctx = (struct socket_ctx*)malloc(sizeof(struct socket_ctx));
		memset(global_socket_ctx, 0x00, sizeof(struct socket_ctx));
		sa_init(global_socket_ctx, 0);
		// sp4_init(global_socket_ctx, 0);
		//
	}
	...
	struct rte_mbuf *m = get_plain_packet_buf(mtcp->ctx);

	// get spi
	int spi = rte_cpu_to_be_16(tcph->dest) - 5990;

	struct rte_ipsec_sa* ipsec_sa = (struct rte_ipsec_sa*)global_socket_ctx->sa_out->ipsec_sa_ptr_list[spi];
		
	int ret;
	// process esp packets
	ret = inline_outb_tun_pkt_process_custom(ipsec_sa, &m, 1);
	
	// encrypt packets
	// int spi = rte_cpu_to_be_32(ipsec_sa->spi);
	if (global_socket_ctx->sa_out->xf[spi].a.type == RTE_CRYPTO_SYM_XFORM_NOT_SPECIFIED) {
		printf("Error2: sa for spi %d not specified\n", spi); fflush(stdout);
		return 0;
	}
	uint8_t *cipherkey = global_socket_ctx->sa_out->xf[spi].a.cipher.key.data;
	uint8_t *authkey = global_socket_ctx->sa_out->xf[spi].b.auth.key.data;
	ret = esp_encrypt_aes_cbc_128_hmac_sha1_96(mtcp->mb_mgr, rte_pktmbuf_mtod(m, char *), m->pkt_len, cipherkey, authkey);
	
	// update ip check sum : ?? cannot understand why it is not being done in inline_outb_tun_pkt_process_custom
	struct iphdr *iph = rte_pktmbuf_mtod(m, struct iphdr *);
	...
	// should be fixed to get the exact mac address of ipsec gateway of client
	...
	char* eth_payload = (char *)EthernetOutput(mtcp, ETH_P_IP, 
			stream->sndvar->nif_out, haddr, m->pkt_len);
	memcpy(eth_payload, rte_pktmbuf_mtod(m, char *), m->pkt_len);	
#endif
	//
